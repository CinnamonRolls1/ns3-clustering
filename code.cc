...
void PhySnirTrace (std::string context ,uint16_t cellId, uint16_t rnti, double rsrp, double sinr, uint8_t componentCarrierId)
{ 
...
int main (int argc, char *argv[])
{
  ...
  *stream->GetStream () << "time(sec)\ttx/rx\tNodeID\tIMSI\tPktSize(bytes)\tIP[src]\tIP[dst]" << std::endl;
  ...
  std::cout<<"\n";
  std::cout<<"    SINR values for Public safety devices (Shard 2)  \n";
  std::cout<<"\n";
  
  Config::Connect ("/NodeList/*/DeviceList/0/$ns3::LteUeNetDevice/ComponentCarrierMapUe/*/LteUePhy/ReportCurrentCellRsrpSinr", MakeCallback (&PhySnirTrace));
